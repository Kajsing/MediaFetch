...
The 0.2.1 source/build adds YouTube title filenames and the current desktop Shorts control. TypeScript/build, 19 extension tests, 42 Windows native tests, content fixtures, real watch/Shorts placement and click identity, and isolated native downloads with full audiovisual validation passed. Installation and normal-Chrome acceptance of 0.2.1 are pending; the installed helper remains 0.2.0 until it can be safely updated. See the Y1.1 record below.
