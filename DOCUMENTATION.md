...
The final storage regression rejects a destination root replaced with a junction after the job was recorded, in addition to rejecting junctions inside staging. This source change passed all 32 native tests. Installation of this final guard is pending until the user's running Chrome acceptance session has released the helper; do not overwrite the active installation.
